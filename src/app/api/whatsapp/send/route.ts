...
export async function POST(request: Request) {
  try {
    ...
    if (isContactMessage && typeof shared_contact_id !== 'string') {
      return NextResponse.json(
        { error: 'shared_contact_id is required for contact messages' },
        { status: 400 }
      );
    }
    ...
    try {
      validateSendMessageParams({
        ...
        // The actual CRM row is resolved below, before a target conversation
        // can be created. This placeholder lets the shared validator enforce
        // the contact-message shape at the same early boundary.
        contactPayload: isContactMessage
          ? { name: 'pending', phone: 'pending' }
          : undefined,
      });
    } catch (err) {
    ...
    let sharedContactPayload:
      ...
      | undefined;
    if (isContactMessage) {
      const { data: sharedContact, error: sharedContactError } = await supabase
        .from('contacts')
        .select('id, name, phone, email, company')
        .eq('id', shared_contact_id)
        .eq('account_id', accountId)
        .maybeSingle();
      if (sharedContactError || !sharedContact?.phone) {
        return NextResponse.json(
          { error: 'Contact to share not found' },
          { status: 404 }
        );
      }
      sharedContactPayload = {
        id: sharedContact.id,
        name: sharedContact.name?.trim() || sharedContact.phone,
    ...
    try {
      const result = await sendMessageToConversation(supabase, accountId, {
        ...
        interactivePayload: interactive_payload,
        contactPayload: sharedContactPayload,
