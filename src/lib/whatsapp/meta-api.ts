...
export interface SendContactMessageArgs {
  ...
  accessToken: string;
  to: string;
  contact: SharedContactMessage;
  contextMessageId?: string;
}

/** Send a WhatsApp contact card, including the fields Meta accepts in vCard. */
export async function sendContactMessage(
  args: SendContactMessageArgs
): Promise<MetaSendResult> {
  const { phoneNumberId, accessToken, to, contact, contextMessageId } = args;
  const formattedName = contact.name.trim();
  const phone = contact.phone.trim();
  if (!formattedName || !phone) {
    throw new Error('Contact messages require a name and phone number.');
  }

  const [firstName, ...restName] = formattedName.split(/\s+/);
  const sharedContact: Record<string, unknown> = {
    name: {
  ...
  if (contact.email?.trim()) {
    sharedContact.emails = [{ email: contact.email.trim(), type: 'WORK' }];
  }
  if (contact.company?.trim()) {
    sharedContact.org = { company: contact.company.trim() };
  }
