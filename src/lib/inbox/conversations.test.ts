...
describe('matchesConversationAssignmentFilter', () => {
  it('keeps all conversations in the all partition', () => {
    const assigned = { ...makeConversation({}), assigned_agent_id: 'agent-1' };
    expect(
      matchesConversationAssignmentFilter(assigned, 'all', 'agent-2')
    ).toBe(true);
  });

  it('matches mine only for the authenticated agent', () => {
    const assigned = { ...makeConversation({}), assigned_agent_id: 'agent-1' };
    expect(
      matchesConversationAssignmentFilter(assigned, 'mine', 'agent-1')
    ).toBe(true);
    expect(
      matchesConversationAssignmentFilter(assigned, 'mine', 'agent-2')
    ).toBe(false);
  });

  it('treats null and legacy undefined assignment as unassigned', () => {
    const unassigned = { ...makeConversation({}), assigned_agent_id: null };
    expect(
      matchesConversationAssignmentFilter(unassigned, 'unassigned', 'agent-1')
    ).toBe(true);
    expect(
      matchesConversationAssignmentFilter(
        makeConversation({}),
