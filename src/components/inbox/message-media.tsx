...
const UNAVAILABLE_LABEL_MARKER = '\uE000emoji-label\uE001';
...
export function MediaUnavailable({
  ...
  // Keep the locale's word order (including Korean's suffix) while replacing
  // only the interpolated user label with safe emoji nodes. `t.rich` is for
  // XML-like message tags, not ordinary `{label}` ICU interpolation.
  const unavailableText = t('unavailable', {
    label: UNAVAILABLE_LABEL_MARKER,
  });
  const labelIndex = unavailableText.indexOf(UNAVAILABLE_LABEL_MARKER);
  ...
  return (
    <div className="bg-muted/40 text-muted-foreground flex items-center gap-2 rounded-lg px-3 py-2 text-xs">
      ...
      <span>
        {labelIndex === -1 ? (
          <EmojiText text={unavailableText} />
        ) : (
          <>
            <EmojiText text={unavailableText.slice(0, labelIndex)} />
            <EmojiText text={label} />
            <EmojiText
              text={unavailableText.slice(
                labelIndex + UNAVAILABLE_LABEL_MARKER.length
              )}
            />
          </>
        )}
    ...
    <div className="flex items-center gap-2">
      ...
      >
        ...
        <span className="truncate">
          <EmojiText text={message.content_text || t('document')} />
        </span>
      </a>
      <MediaActionButton
        icon={Download}
