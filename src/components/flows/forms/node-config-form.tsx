        ...
        {sections.map((section, sIdx) => (
          ...
          >
            ...
            {section.rows.map((row, rIdx) => (
              <div
                key={rIdx}
                className="border-border/70 bg-card/60 mb-2 rounded-lg border p-2.5"
              >
                <div className="flex items-start gap-2">
                  <div className="min-w-0 flex-1">
                    <label className="text-muted-foreground mb-1 block text-xs">
...
function ConditionForm({
  ...
  const contactFieldLabel =
    cfg.subject_key === 'name'
      ? t('contactFieldName')
      : cfg.subject_key === 'email'
        ? t('contactFieldEmail')
        : cfg.subject_key === 'phone'
          ? t('contactFieldPhone')
          : cfg.subject_key === 'company'
        ...
        <div className="@2xl:col-span-2">
          ...
          {subject === 'tag' && tags.length > 0 ? (
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="name">{t('contactFieldName')}</SelectItem>
                <SelectItem value="email">{t('contactFieldEmail')}</SelectItem>
                <SelectItem value="phone">{t('contactFieldPhone')}</SelectItem>
      ...
      if (file.size > MEDIA_MAX_BYTES) {
        toast.error(
          t('fileTooLarge', {
            ...
            limit: (MEDIA_MAX_BYTES / 1024 / 1024).toFixed(0),
          })
        );
        return;
      }
