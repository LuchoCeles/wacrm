...
export function QuickRepliesManager() {
  ...
  const save = useCallback(async () => {
    if (!draft) return;
    if (!draft.title.trim()) {
      toast.error("Asigná un nombre a la respuesta rápida.");
      return;
    }
    const payload =
  ...
  return (
    <div>
      <SettingsPanelHead
        ...
        description="Fragmentos reutilizables, texto simple o mensajes interactivos guardados, que los agentes pueden insertar desde el redactor de la bandeja de entrada."
        action={
      ...
      {loading ? (
        ...
        <p className="rounded-lg border border-dashed border-border py-10 text-center text-sm text-muted-foreground">
          Aún no hay respuestas rápidas. Creá una para reutilizarla en las conversaciones.
        </p>
      ) : (
      ...
      <Dialog open={!!draft} onOpenChange={(o) => !o && setDraft(null)}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{draft?.id ? "Editar respuesta rápida" : "Nueva respuesta rápida"}</DialogTitle>
          </DialogHeader>
          {draft && (
